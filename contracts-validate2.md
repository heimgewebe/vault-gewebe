...
jobs:
  version-sync-check:
    ...
    steps:
      ...
      - name: Verify 'uses:' pins for heimgewebe/contracts
        ...
        run: |
          set -euo pipefail
          shopt -s extglob  # für +([[:space:]]) Trims
          ...
          # Extrahiert den Wert hinter 'uses:' (ein- oder doppel-quoted oder unquoted)
          extract_pair() {
            local line="$1"
            line="${line%%#*}"
            line="${line##+([[:space:]])}"
          ...
          # Prüft Repo, verbietet dynamische Refs, erzwingt exakten Pin
          check_pair() {
    ...
    steps:
      ...
      - name: Enforce guard policy
        run: |
          ...
          # Optionaler Early-Exit: Removals global erlaubt
          if is_truthy "$ALLOW_REMOVALS"; then
          ...
          # Fallback: Default-Branch Merge-Base
          if [[ -z "$base" ]]; then
          ...
          # Nur Deletions und Renames einlesen; robust gegen Tabs in Pfaden (Tab-separiert)
          ...
          done < <(git diff --name-status --diff-filter=DR "${base}...HEAD") > "$blocked_file"
  ...
  validate:
    ...
    secrets: inherit
    with:
      # Standard: nur "gültige" Fixtures validieren; per Repository-Variable übersteuerbar
