// d3-renderer.js — Version: Σ.v∞.3.d3-renderer.audit.full.stable

import { generateEreignisTooltipContent } from './utility-core.js';
import {
    initTooltip,
    showTooltip,
    hideTooltip,
...
import { antragColors } from './colors.js';
...
// ------------------------- //
// Render Ereignisse (Tortendiagramme + Antragsmarker)

async function renderEreignisse(selection, projection, bounds, data) {
    const getLatLng = e =>
    ...
    // NEU: Marker für Anträge integrieren:
    for (const ereignis of visible) {
        const raumId = 'ereignis-' + ereignis.id;
        const raum = await diskursApi.getRaum(raumId).catch(() => null);
        if (!raum) continue;

        const offeneThreads = raum.threads.filter(t => t.antrag);
        offeneThreads.forEach(t => {
            const status = t.antrag?.status;
            if (!status) return;

            const [lat, lng] = getLatLng(ereignis);
            const pos = projection({ lat, lng });

            selection.append('circle')
                .attr('class', 'antrags-marker')
                .attr('r', 5)
                .attr('cx', pos.x + 12)
                .attr('cy', pos.y - 12)
                .style('fill', antragColors[status] || '#666');
        });
    }
}